/* tslint:disable */
/* eslint-disable */
// @generated
// This file was automatically generated and should not be edited.

import { RestaurantInput } from "./globalTypes";

// ====================================================
// GraphQL query operation: restaurantQuery
// ====================================================

export interface restaurantQuery_restaurant_result_category {
  __typename: "Category";
  name: string;
  slug: string;
}

export interface restaurantQuery_restaurant_result_menu_options_choices {
  __typename: "DishChoice";
  name: string;
  price: number | null;
}

export interface restaurantQuery_restaurant_result_menu_options {
  __typename: "DishOption";
  name: string;
  price: number | null;
  choices: restaurantQuery_restaurant_result_menu_options_choices[] | null;
}

export interface restaurantQuery_restaurant_result_menu {
  __typename: "Dish";
  id: number;
  name: string;
  photo: string | null;
  price: number;
  description: string;
  options: restaurantQuery_restaurant_result_menu_options[] | null;
}

export interface restaurantQuery_restaurant_result {
  __typename: "Restaurant";
  id: number;
  name: string;
  address: string;
  coverImg: string | null;
  category: restaurantQuery_restaurant_result_category | null;
  isPromoted: boolean;
  menu: restaurantQuery_restaurant_result_menu[];
}

export interface restaurantQuery_restaurant {
  __typename: "RestaurantOutput";
  ok: boolean;
  error: string | null;
  result: restaurantQuery_restaurant_result | null;
}

export interface restaurantQuery {
  restaurant: restaurantQuery_restaurant;
}

export interface restaurantQueryVariables {
  input: RestaurantInput;
}
