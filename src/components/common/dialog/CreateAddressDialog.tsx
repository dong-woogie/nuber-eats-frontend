...
import { CREATE_ADDRESS_MUTATION } from "../../../lib/graphql/user";
import {
  createAddress,
...
function CreateAddressDialog() {
  const addressDialog = useReactiveVar(createAddressDialogVars);
  ...
  const onClose = () => {
    createAddressDialogVars(false);
  };
  const onSubmit = handleSubmit(() => {
    const { detailAddress } = getValues();
    createMutation({
      variables: { input: { address, detailAddress } },
    });
  });
